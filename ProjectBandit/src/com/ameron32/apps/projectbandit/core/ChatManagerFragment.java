...
public class ChatManagerFragment extends
    ContentFragment 
    implements
  ...
  private void initChatViewsWithPager() {
    ...
    // int position =
    // mChatViewPagerAdapter.getViewPositionFromId(currentChatViewId);
    // mViewPager.setCurrentItem(mCurrentChatViewPosition);
  ...
  private Message makeMessage(
    ...
    return Message.create().setMessage(message).setChannel(channel).setCharacter(CharacterManager.get().getChatCharacter()).setAction(currentAction);
  }
